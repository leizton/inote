...
class Processor (processorId: Int,
	...
	// Selector 的构造函数里 $.nioSelector = java.nio.channels.Selector.open()
	...
	override def run() {  // 实现-Runnable
		...
		while super.isRunning
			...
			$.selector.poll(300)     // step-2 建立连接，读、写数据(经握手验权后建立新连接，可读时读取一个完整的NetworkReceive，可写时写Send)
	...
	def configureNewConnections() {
		while !newConnections.isEmpty
			val socketChannel = $.newConnections.poll()  // client channel
			...
			$.selector.register(connectionId, socketChannel)
				$.selector.registerChannel(connectionId, socketChannel, SelectionKey.OP_READ)
					$.selector.channels.put(connectionId, socketChannel)
	}
	...
	def processCompletedReceives() {
		$.selector.completedReceives.foreach(NetworkReceive receive)
			...
			try {
				...
				$.selector.mute(kChannelId)  // kafkaChannel.mute()，暂停接收数据
			}
			catch Exception e {
				$.close(kChannelId)
			}
	...
	def processNewResponses() {
		...
		// 尝试从队列中取Response
		while ( curr = $.requestChannel.responseQueues($.processorId).poll() ) != null
			curr.responseAction match {
				...
				case RequestChannel.CloseConnectionAction =>
					$.close(curr.request.connectionId)  // 关闭channel
			}
	}
	...
	def processCompletedSends() {
		...
		$.selector.completedSends.foreach(Send send)
			$.selector.unmute(send.destination/* kafkaChannel id */)  // kafkaChannel.unmute()，恢复接收数据
	}
...
> register(String channelId, SocketChannel socketChannel)
	...
	$.channels.put(channelId, kafkaChannel)
...
> poll(long timeout)
	...
	int ready = select(timeout)
		=> return timeout == 0L ? $.nioSelector.selectNow() : $.nioSelector.select(timeout)
	//
	if ready > 0 || !$.immediatelyConnectedKeys.isEmpty
		pollSelectionKeys($.nioSelector.selectedKeys(), false, endSelect)
		pollSelectionKeys($.immediatelyConnectedKeys, true, endSelect)
	//
	addToCompletedReceives()

> clear()
	$.completedSends/completedReceives/connected/disconnected.clear()
...
> pollSelectionKeys(Iterable<SelectionKey> selectionKeys, boolean isImmediatelyConnected, long currentTimeNanos)
	...
	while it.hasNext()
		...
		try
			...
			if channel.ready() && key.isReadable() && !$.stagedReceives.containsKey(channel)
			...
			if channel.ready() && key.isWritable()
				...
				if send != null
					$.completedSends.add(send)  // send已经完整地发送出去
			if !key.isValid()
				close(channel, true)
		catch Exception e
			close(channel, true)
 ...
 */
> addToCompletedReceives()
	if $.stagedReceives.isEmpty()
		return
	it = stagedReceives.entrySet().iterator()
	while it.hasNext()
		KafkaChannel channel, Deque<NetworkReceive> deque = it.next()
		if !channel.isMute()
			$.completedReceives.add(deque.poll())
			if deque.isEmpty()
				it.remove()

> unmute(String channelId)
	KafkaChannel ch = $.channels.get(channelId)
	if ch == null
		ch = $.closingChannels.get(channelId)
	if ch != null
		ch.unmute()

> send(Send send)
	if $.closingChannels.containsKey(send.destination)
		$.failedSends.add(send)
	else
		// channel会添加{OP_WRITE}事件
		$.channels.get(send.destination).setSend(send)